...
va_ookla_bg_2 <- st_drop_geometry(va_ookla_bg)  %>%
  ...
  mutate(total_percent = sum(bg_percent, na.rm = T),
         ...
         avg_download_by_devices = sum(avg_d_kbps * devices) / sum(devices),
         avg_download_by_tests = sum(avg_d_kbps * tests) / sum(tests),
         avg_upload_by_devices = sum(avg_u_kbps * devices) / sum(devices),
         avg_upload_by_tests = sum(avg_u_kbps * tests) / sum(tests),
         avg_lat_by_devices = sum(avg_lat_ms * devices) / sum(devices),
         avg_lat_by_tests = sum(avg_lat_ms * tests) / sum(tests)) %>%
  summarise(download_devices = sum(avg_download_by_devices * frac_devices * true_percent * tile_percent / denom_devices, na.rm = T) / 1000,
            download_tests = sum(avg_download_by_tests * frac_tests * true_percent * tile_percent / denom_tests, na.rm = T) / 1000,
            upload_devices = sum(avg_upload_by_devices * frac_devices * true_percent * tile_percent / denom_devices, na.rm = T) / 1000,
            upload_tests = sum(avg_upload_by_tests * frac_tests * true_percent * tile_percent / denom_tests, na.rm = T) / 1000,
            latency_devices = sum(avg_lat_by_devices * frac_devices * true_percent * tile_percent / denom_devices, na.rm = T),
            latency_tests = sum(avg_lat_by_tests * frac_tests * true_percent * tile_percent / denom_tests, na.rm = T),
